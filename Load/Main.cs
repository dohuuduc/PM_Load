...
namespace Load {
  ...
  public partial class Main : Form {
    ...
    private CancellationTokenSource _cancelTokenVinabiz;
    private void button8_Click(object sender, EventArgs e) {
      try {
        ...
        if (btnvinaStartNew.Text == "Start") {
          btnvinaStartNew.Text = "Stop";
          setTitleWindow(7, true);
          btn_vinabiz_Them.Enabled = false;
          btn_vinabiz_Xoa.Enabled = false;
          btn_vinabiz_Bo.Enabled = false;
          ...
          btnvinaStartNew.Text = "Start";
          _cancelTokenVinabiz.Cancel();
          setTitleWindow(7, false);
        ...
        MessageBox.Show(ex.Message, "button8_Click");
      }
  }
    async void vinabizStart() {
      try {
        ArrayList arr = new ArrayList() { lblmessagevina, lbl_vinabiz_khoa,lbl_thanhcong_vinabiz,lbl_thatbai_vinabiz };
        ...
        List<Gridview> models = new List<Gridview>();
        foreach (DataGridViewRow row in gw_vinabiz_chon.Rows) {
          int id = Convert.ToInt32(row.Cells["id_vnbiz_chon"].Value);
          string strPath = row.Cells["path_vnbiz_chon"].Value.ToString();
          string strName = row.Cells["name_vnbiz_chon"].Value.ToString();
          int pathlimit = Convert.ToInt32(row.Cells["end_chon"].Value);

          Gridview temp = new Gridview();
          temp.id = id;
          temp.path = strPath;
          temp.name = strName;
          temp.limit = pathlimit;
          models.Add(temp);
        }
        Helpers.shownText = lblmessagevina;
        Helpers.addText(string.Format("Số lượng quét: {0}", models.Count()));
        //https://talkdotnet.wordpress.com/tag/maxdegreeofparallelism/
        ParallelOptions po = new ParallelOptions();
        //Create a cancellation token so you can cancel the task.
        _cancelTokenVinabiz = new CancellationTokenSource();
        po.CancellationToken = _cancelTokenVinabiz.Token;
        ...
        await Task.Run(() => {
          Parallel.ForEach(models, po, model => {
            Utilities_vinabiz._PathLimit = model.limit;
            string strUrl = model.path.Replace(" ", "").Replace("\t", "");
            infoPathmuaban pagemax = Utilities_vinabiz.getPageMax(strUrl, arr);
            DataTable tblFiltered = _table_vinabiz.AsEnumerable()
                                        .Where(row => row.Field<int>("id_chon") == model.id)
                                        .CopyToDataTable();
            if (tblFiltered != null && tblFiltered.Rows.Count != 0)
              pagemax.PageNow = ConvertType.ToInt(tblFiltered.Rows[0]["trang_chon"]);
            pagemax.TotalPagingMax = model.limit == -1 ? pagemax.TotalPagingMax : model.limit;

            // Check for cancellation here
            _cancelTokenVinabiz.Token.ThrowIfCancellationRequested();
            if (_cancelTokenVinabiz.IsCancellationRequested) {
              Helpers.addText(string.Format("\nTask:{0}-Luồng:{1} ---->Tạm Dừng Người Dùng\n", ConvertType.ToInt(Task.CurrentId).ToString("D2"), Thread.CurrentThread.ManagedThreadId.ToString("D2")));
              return;
            }
            ...
            do {
              ...
              int solanlap = 0;
              model.path = strUrl1;/*update link*/
              Helpers.addText(string.Format("\nTask:{0}-Luồng:{1} Page:{2}~{3}->'{4}'\n-> {5}", ConvertType.ToInt(Task.CurrentId).ToString("D2"), Thread.CurrentThread.ManagedThreadId.ToString("D2"), i, pagemax.TotalPagingMax, model.name, model.path));
              Utilities_vinabiz.IdDanhmuc = model.id;
              Utilities_vinabiz.getwebBrowser(model, ref solanlap, arr);
              i = i + 1;
            } while (i <= pagemax.TotalPagingMax);
            if (!_cancelTokenVinabiz.IsCancellationRequested) {
