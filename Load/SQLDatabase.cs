  ...
  class SQLDatabase {
    ...
    public static bool Adddm_vinabiz_LoaiDinhNghia(dm_vinabiz_LoaiDinhNghia record) {
      ...
      try {
        ...
        cmd.Connection = cnn;
        //--- Insert Record
        cmd.CommandText = "Insert into dm_vinabiz_LoaiDinhNghia( name, orderid,isAct)" +
                            "values( @name,  @orderid,@isAct);" +
                            "Select SCOPE_IDENTITY();";
      ...
      try {
        ...
        while (reader.Read()) {
          ...
          if (!reader.IsDBNull(5))
            InfoCOMMANDTABLE.orderid = reader.GetInt32(5);
          if (!reader.IsDBNull(6))
            InfoCOMMANDTABLE.isLoai = reader.GetInt32(6);
          InfoCOMMANDTABLEs.Add(InfoCOMMANDTABLE);
        }
        return InfoCOMMANDTABLEs;
      }
    ...
    public static bool AdddmVinabiz(dm_vinabiz record) {
      ...
      try {
        ...
        cmd.Connection = cnn;
        //--- Insert Record
        cmd.CommandText = "Insert into dm_vinabiz( name, parentId, path, orderid,isloai)" +
                            "values( @name, @parentId, @path, @orderid,@isloai);" +
                            "Select SCOPE_IDENTITY();";
    ...
    public static bool Updatedm_vinabiz(dm_vinabiz record) {
      ...
      try {
        ...
        cmd.CommandText = "Update [dm_vinabiz] Set name=@name, "
                            + " path=@path,parentId=@parentId,orderid=@orderid,isloai=@isloai "
                            + " where ID='" + record.id + "'";
        cmd.CommandType = CommandType.Text;
