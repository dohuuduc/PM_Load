...
namespace Load {
  public partial class frm_dmVinabiz : Form {
    ...
    private void getdata() {
      _toancuc = SQLDatabase.ExcDataTable(" WITH temp(id, name,path,parentId,orderid, alevel)  as (  "+
                                                  " Select id, name, path,parentId, orderid,0 as aLevel  "+
                                                  " From dm_vinabiz  Where parentId is null  "+
                                                  "Union All  "+
                                                  " Select b.id, b.name, b.path,b.parentId,b.orderid, a.alevel + 1 "+
                                                  " From temp as a, dm_vinabiz as b  Where a.id = b.parentId  )  Select *  From temp ");
    }
